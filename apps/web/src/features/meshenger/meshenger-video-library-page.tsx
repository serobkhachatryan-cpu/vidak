...
export function MeshengerVideoLibraryPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const body = (await response.json()) as { items?: LibraryVideo[]; error?: { message?: string } };
      if (!response.ok || !Array.isArray(body.items)) {
        throw new Error(body.error?.message ?? 'Meshenger videos are unavailable.');
      }
      setState({ status: 'ready', items: body.items });
    } catch (error) {
      setState({ status: 'error', message: error instanceof Error ? error.message : 'Meshenger videos are unavailable.' });
    }
  }, []);
  ...
  return (
    <ApplicationShell currentHref="/meshenger">
      <Page
        ...
        actions={<Button variant="secondary" onClick={() => void load()}>Refresh library</Button>}
      >
        {state.status === 'loading' ? (
          ...
          <ErrorState title="Could not load Meshenger videos" description={state.message} retry={() => void load()} />
        ) : state.items.length === 0 ? (
          <EmptyState title="No Meshenger videos found" description="When videos are available in your W3DS vault, they will appear here." />
        ) : (
          <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3">
            {state.items.map((video) => (
              <article key={video.id} className="overflow-hidden rounded-xl border border-border bg-surface-raised">
                <video
                ...
                <div className="space-y-1 p-4">
                  <p className="text-xs font-semibold uppercase tracking-wide text-primary">{label(video)}</p>
...
function label(video: LibraryVideo): string {
  ...
  if (video.kind === 'video-message') return video.shape === 'circle' ? 'Video circle' : 'Video message';
  return 'Video file';
}
