...
Release 4 now supports immediate public YouTube channel links. A person can paste a public channel ID or canonical channel URL; Vidak reads the public YouTube feed, stores only catalogue metadata, and plays through YouTube’s privacy-enhanced embed. The Settings and linked-channel library form one continuous flow. Owner-authorized YouTube and Vimeo connections remain intentionally hidden until their server-only OAuth credentials are configured; no disabled Connect actions are presented as working integrations.
